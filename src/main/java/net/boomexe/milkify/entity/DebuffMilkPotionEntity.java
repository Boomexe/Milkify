...
public class DebuffMilkPotionEntity extends ThrownItemEntity implements FlyingItemEntity {
    ...
    public DebuffMilkPotionEntity(EntityType<? extends PotionEntity> entityType, World world) {
        super(entityType, world);
    }

    public DebuffMilkPotionEntity(World world, LivingEntity owner) {
        super(EntityType.POTION, owner, world);
    }

    public DebuffMilkPotionEntity(World world, double x, double y, double z) {
    ...
    protected void onCollision(HitResult hitResult) {
        ...
        if (!this.world.isClient) {
            ...
            this.world.syncWorldEvent(2002, this.getBlockPos(), 14679911);
            this.discard();
            if (this.isLingering()) {
                this.applyLingeringPotion(itemStack, potion);

                int i = potion.hasInstantEffect() ? 2007 : 2002;
                this.world.syncWorldEvent(i, this.getBlockPos(), 14679911);
                this.discard();
            } else {
                this.applySplashPotion(list, hitResult.getType() == HitResult.Type.ENTITY ? ((EntityHitResult)hitResult).getEntity() : null);
            }
